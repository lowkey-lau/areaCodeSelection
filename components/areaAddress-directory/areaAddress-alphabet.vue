...
<script>
	export default {
		name:"areaAddress-alphabet",
		props:{
			areaAddresss:Object,
			areaAddressListIndex:String
		},
		data() {
			return {
				touchStatus:false,
		...
		mounted () {
			let areaAddresssArr = Object.keys(this.areaAddresss)
			this.areaAddresssArr = areaAddresssArr
		},
		watch:{
			areaAddressListIndex (index) {
			...
			handleTouchMove (e) {
				...
				if(this.touchStatus){
					...
					this.timer = setTimeout(()=>{
						...
						if(index >= 0 && index < this.areaAddresssArr.length){
							this.activeClass = this.areaAddresssArr[index]
							this.$emit('change',this.areaAddresssArr[index])
						} 
					},16)
				}
			},
